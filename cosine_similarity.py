...
df_aa_pred = pd.read_csv('pred_analysis_results/iter_aa_composition_analysis.csv', index_col=False)
df_atomic_pred = pd.read_csv('pred_analysis_results/iter_atomic_composition_analysis.csv', index_col=False)
# print(df_protparam_pred.info())
# print(df_expasy_pred.info())
print(df_aa_pred.describe())
...
df_aa_iedb = pd.read_csv('analysis_iedb_results/iter_aa_composition_analysis.csv', index_col=False)
df_atomic_iedb = pd.read_csv('analysis_iedb_results/iter_atomic_composition_analysis.csv', index_col=False)
...
# print(summary)
# print(results)
print(stats.ttest_ind(df_aa_iedb[df_aa_iedb.columns[4]],
                df_aa_pred[df_aa_pred.columns[4]]))


# result = 1 - spatial.distance.cosine(ds1_sample, iedb_aliphatic_index)
# print(result)
pred_df = reduce(lambda x,y: pd.merge(x,y, on='peptide', how='outer'), [df_expasy_pred, df_protparam_pred])
#print(pred_df)
iedb_df = reduce(lambda x,y: pd.merge(x,y, on='peptide', how='outer'), [df_expasy_iedb, df_protparam_iedb])
...
#         result = 1 - spatial.distance.cosine(pred_epitope_parameters[1:], iedb_epitope_parameters[1:])
#         #print(result)
#         if result >= 0.9:
